...
impl MemTable {
    ...
    pub fn inc_ref(&mut self) { self.refs += 1; }
    ...
    pub fn add(&self, seq: SequenceNumber, val_ty: ValueType, key: &Slice, val: &Slice) {
        ...
        let encoded_len = coding::varint_length(internal_key_size as u64)
            + internal_key_size
            + coding::varint_length(val_size as u64)
            + val_size;
        let mut buf = unsafe {
        ...
        offset += 8;
        let val_encoded_len =
            coding::encode_varint_32(&mut buf[offset..], val_size as u32);
    ...
    pub fn get(&self, key: &LookupKey) -> Option<Result<String>> {
        ...
        if iter.valid() {
            ...
            let (key_length_32, length) = coding::decode_varint_32_limit(entry, 5)
            ...
            if self
                .comparator
                .user_comparator()
                .compare(&Slice::from(user_key), &key.user_key())
                == Ordering::Equal
            {
                // Correct user key
                ...
                match ValueType::try_from((tag & 0xff) as u8).expect("invalid tag") {
                    ...
                    ValueType::DELETION => return Some(LEVELDB_ERR!(NotFound)),
                }
            }
        }
        None
