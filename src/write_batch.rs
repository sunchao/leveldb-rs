...
use dbformat::{ValueType, SequenceNumber};
...
impl WriteBatch {
  ...
  ///
  /// This number is tied to implementation details, and may change across releases. It is
  /// intended for LevelDB usage metrics.
...
impl WriteBatchInternal {
  pub fn count(b: &WriteBatch) -> u32 {
    coding::decode_fixed_32(&b.rep[8..])
  }
  ...
  pub fn contents(b: &mut WriteBatch) -> &mut Vec<u8> {
    &mut b.rep
  }

  pub fn insert_into(b: &WriteBatch, mem: Rc<MemTable>) -> Result<()> {
    let mut inserter = Box::new(MemTableInserter::new(
      WriteBatchInternal::sequence(b), mem)) as Box<Handler>;
...
impl MemTableInserter {
  pub fn new(seqno: SequenceNumber, mem: Rc<MemTable>) -> Self {
    Self { seqno: seqno, mem: mem }
  }
}
...
impl Handler for MemTableInserter {
  ...
  fn delete(&mut self, key: &Slice) {
    self.mem.add(self.seqno, ValueType::DELETION, key, &Slice::new_empty());
    self.seqno += 1;
  }
}
...
mod tests {
  ...
  fn test_append() {
    ...
    WriteBatchInternal::append(&mut b1, &b2);
    assert_eq!(print_contents(&b1), // TODO: the order is different from cpp, visit again.
               "Put(a, va)@200\
                Put(b, vb)@201\
                Put(b, vb)@202\
                Delete(foo)@203");
