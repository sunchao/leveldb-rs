...
impl Writer {
  ...
  pub fn add_record(&mut self, slice: &Slice) -> Result<()> {
    ...
    loop {
      ...
      if leftover < HEADER_SIZE {
        ...
        if leftover > 0 {
          ...
          let slice = Slice::from(
            &"\x00\x00\x00\x00\x00\x00"[..leftover as usize]);
      ...
      let ty: RecordType =
        if begin && end { RecordType::FULL }
        else if begin { RecordType::FIRST }
        else if end { RecordType::LAST }
  ...
  fn init_type_crc(type_crc: &mut [u32]) {
    for h in 0..MAX_RECORD_TYPE+1 {
      let v: [u8; 1] = unsafe { mem::transmute(RecordType::try_from(h).unwrap() as u8) };
...
mod tests {
  ...
  impl LogTest {
    ...
    pub fn write<'a>(&mut self, msg: &'a str) {
      ...
      self.writer.add_record(&Slice::from(msg)).expect("add_record() should be OK");
    }
    ...
    pub fn fix_checksum(&mut self, header_offset: usize, len: usize) {
      ...
      let mut crc = crc32c::value(&contents[header_offset+6..header_offset+7+len]);
      crc = crc32c::mask(crc);
    ...
    pub fn check_initial_offset_record(
      ...
      let mut offset_reader = Reader::new(
        self.source.clone(), Some(self.reporter.clone()), true, initial_offset
      );
      ...
      loop {
        ...
        let mut scratch = Vec::new();
        let record = offset_reader.read_record(&mut scratch)
          .expect("read_record() should be OK");
        assert_eq!(INITIAL_OFFSET_RECORD_SIZES[expected_record_offset], record.size());
        assert_eq!(INITIAL_OFFSET_LAST_RECORD_OFFSETS[expected_record_offset],
    ...
    pub fn check_offset_past_end_returns_no_records(&mut self, offset_past_end: u64) {
      ...
      let mut offset_reader = Reader::new(
        self.source.clone(), Some(self.reporter.clone()), true,
  ...
  pub fn read_fourth_start() {
    ...
    test.check_initial_offset_record(
      (2 * (HEADER_SIZE + 10000) + (2 * BLOCK_SIZE - 1000) + 3 * HEADER_SIZE) as u64, 3);
  }
