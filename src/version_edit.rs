...
impl VersionEdit {
  ...
  pub fn encode_to(&self, dst: &mut Vec<u8>) {
    if self.has_comparator {
      ...
      coding::encode_length_prefixed_slice(dst, &Slice::from(self.comparator.as_str()));
    }
    ...
    loop {
      ...
      if let Ok(tag) = Tag::try_from(tag_num.unwrap() as u8) {
        match tag {
          ...
          Tag::LogNumber => {
            if let Some(log_number) = coding::decode_varint_64_slice(&mut input) {
              self.has_log_number = true;
          ...
          Tag::PrevLogNumber => {
            if let Some(prev_log_number) = coding::decode_varint_64_slice(&mut input) {
              self.has_prev_log_number = true;
          ...
          Tag::NextFileNumber => {
            if let Some(next_file_number) = coding::decode_varint_64_slice(&mut input) {
              self.has_next_file_number = true;
          ...
          Tag::LastSequence => {
            if let Some(last_sequence) = coding::decode_varint_64_slice(&mut input) {
              self.has_last_sequence = true;
          ...
          Tag::CompactPointer => {
            ...
            if maybe_level.is_some() && maybe_key.is_some() {
              self.compact_pointers.push((maybe_level.unwrap(), maybe_key.unwrap()));
            } else {
          ...
          Tag::DeletedFile => {
            ...
            if maybe_level.is_some() && maybe_num.is_some() {
              self.deleted_files.insert((maybe_level.unwrap(), maybe_num.unwrap()));
            } else {
...
mod tests {
  ...
  fn test_encode_decode(edit: &VersionEdit) {
    ...
    parsed.decode_from(&Slice::from(&encoded)).expect("decode_from() should be OK");
